...
SELECT  o.ID, o.进货日期, o.货号, o.商品图片, o.进货数量, o.店铺, o.JD订单号, o.客人地址, o.淘宝账号, o.淘宝订单号, o.采购备注, o.订单修改备注, o.来快递, 
               ...
               o.是否淘宝退回, o.售后完结, o.客服ID, o.客服时间, o.高亮, cg.Name AS [采购人.Name], sd.Name AS [审单操作.Name], cb.Name AS [拆包人.Name], 
               sho.Name AS [售后操作.Name], sho.SN AS [售后操作.SN], shr.Name AS [售后原因.Name], shr.SN AS [售后原因.SN], sh.Name AS [售后人员.Name], 
               gj.Name AS [跟进人.Name], kf.Name AS [客服.Name], CAST(CASE WHEN o.[发货时间] IS NULL THEN 0 ELSE 1 END AS BIT) AS 已跟进, 
               CAST(CASE WHEN o.[导入时间] IS NULL THEN 0 ELSE 1 END AS BIT) AS 已导入, CAST(CASE WHEN o.[拆包时间] IS NULL THEN 0 ELSE 1 END AS BIT) 
               AS 已拆包, CAST(CASE WHEN o.售后操作ID IS NULL THEN 0 ELSE 1 END AS BIT) AS 已售后
